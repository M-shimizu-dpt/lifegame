	...
	public void reloadInfo() {
		...
		if(ContainsEvent.isEffect()){
			if(Player.player.getEffect()==-3) {
				mainInfo.setText("自社情報 "+"名前："+Player.player.getName()+" 持ち金："+FrameEvent.convertMoney(Player.player.getMoney())+" "+App.year+"年目 "+App.month+"月 "+Japan.getGoalName()+"まで"+Ginga.getGoalDistance()+"マス 牛歩ｶｰﾄﾞ効果発動中");
			}else {
