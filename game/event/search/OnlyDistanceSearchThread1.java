...
public class OnlyDistanceSearchThread1 extends OnlyDistanceSearchThreadModel{

	//start
	public OnlyDistanceSearchThread1() {
		this.player=Player.getPlayer(0);
		...
		OnlyDistanceSearchThread1.time = System.currentTimeMillis();
		OnlyDistanceSearchThread1.allCount=500;
		OnlyDistanceSearchThread1.initSearchTime();
	}

	//start
	public OnlyDistanceSearchThread1(int searchTime) {
		this.player=Player.getPlayer(0);
		this.start.setValue(this.player.getNowMass());
		this.player.initGoalDistance();
		setMass(player.getNowMass());
		this.setPriority(MAX_PRIORITY);
		japan.getCoordinates(nowMass).open(0);
		...
		OnlyDistanceSearchThread1.time = System.currentTimeMillis();
		OnlyDistanceSearchThread1.allCount=500;
		OnlyDistanceSearchThread1.initSearchTime(searchTime);
	}

	//continue
	public OnlyDistanceSearchThread1(OnlyDistanceSearchThread1 original) {
		threadCopy(original);
	}
	...
	public void run() {
		...
		while(count<=OnlyDistanceSearchThread1.allCount && count <= 40 && System.currentTimeMillis()-OnlyDistanceSearchThread1.time<=searchTime) {
			synchronized(OnlyDistanceSearchThread1.lock2) {
			...
			synchronized(OnlyDistanceSearchThread1.lock1) {
			...
			for(Coordinates coor:list) {//移動処理
				if(me) {
					...
					//Threadを立ち上げ、移動する
					OnlyDistanceSearchThread1 thread = new OnlyDistanceSearchThread1(this);
	...
	@Override
	protected void goal() {
		synchronized(OnlyDistanceSearchThread1.lock3) {
