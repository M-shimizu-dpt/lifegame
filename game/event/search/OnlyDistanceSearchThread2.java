...
public class OnlyDistanceSearchThread2 extends OnlyDistanceSearchThreadModel{

	//start
	public OnlyDistanceSearchThread2() {
		...
		OnlyDistanceSearchThread2.time = System.currentTimeMillis();
		OnlyDistanceSearchThread2.allCount=500;
		OnlyDistanceSearchThread2.initSearchTime();
	}

	//start
	public OnlyDistanceSearchThread2(int searchTime) {
		this.player=Player.getPlayer(1);
		super.start.setValue(this.player.getNowMass());
		this.player.initGoalDistance();
		setMass(player.getNowMass());
		...
		OnlyDistanceSearchThread2.time = System.currentTimeMillis();
		OnlyDistanceSearchThread2.allCount=500;
		OnlyDistanceSearchThread2.initSearchTime(searchTime);
	}

	//continue
	public OnlyDistanceSearchThread2(OnlyDistanceSearchThread2 original) {
		threadCopy(original);
	}
	...
	public void run() {
		...
		while(count<=OnlyDistanceSearchThread2.allCount && count <= 40 && System.currentTimeMillis()-OnlyDistanceSearchThread2.time<=searchTime) {
			synchronized(OnlyDistanceSearchThread2.lock2) {
			...
			synchronized(OnlyDistanceSearchThread2.lock1) {
			...
			for(Coordinates coor:list) {//移動処理
				if(me) {
					...
					//Threadを立ち上げ、移動する
					OnlyDistanceSearchThread2 thread = new OnlyDistanceSearchThread2(this);
	...
	@Override
	protected void goal() {
		synchronized(OnlyDistanceSearchThread2.lock3) {
